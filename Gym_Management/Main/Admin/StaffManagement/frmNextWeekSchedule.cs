...
{
    public partial class frmNextWeekSchedule : Form
    {
        private DBHelper db = new DBHelper();

        public frmNextWeekSchedule()
        {
            InitializeComponent();
        }

        private void frmNextWeekSchedule_Load(object sender, EventArgs e)
        {
        ...
        private void LoadNextWeekSchedule()
        {
            try
            {
                DateTime startWeek = GetStartOfWeek(DateTime.Now).AddDays(7);
                DateTime endWeek = startWeek.AddDays(6);

                lblWeek.Text = $"Tuần tới: {startWeek:dd/MM/yyyy} - {endWeek:dd/MM/yyyy}";
            ...
            {
                MessageBox.Show("Lỗi khi tải lịch tuần tới: " + ex.Message);
            }
        }
